...
public abstract class SlicedLivingEntity extends SlicedEntity {
    ...
    if (!source.isIn(DamageTypeTags.BYPASSES_ARMOR)) {
      amount =
          DamageUtil.getDamageLeft(
              (LivingEntity) this.entityBridge,
              amount,
              source,
              this.getArmor(),
  ...
  public Vec3d applyFluidMovingSpeed(double gravity, boolean falling, Vec3d motion) {
    if ((gravity != 0.0) && (!this.isSprinting())) {
      double d = 0.0;
      if ((falling && (Math.abs(motion.y - 0.005) >= 0.003))
          && (Math.abs(motion.y - (gravity / 16.0)) < 0.003)) {
      } else {
    ...
    if ((this.horizontalCollision || this.jumping)
        && (this.isClimbing()
            || (this.wasInPowderSnow
                && PowderSnowBlock.canWalkOnPowderSnow((LivingEntity) this.entityBridge)))) {
  ...
  protected void becomeAngry(DamageSource damageSource) {
    if (((damageSource.getAttacker() instanceof LivingEntity livingEntity)
        ...
        && ((!damageSource.isOf(DamageTypes.WIND_CHARGE))
            || (!this.getType().isIn(EntityTypeTags.NO_ANGER_FROM_WIND_CHARGE)))) {
  ...
  protected void dropExperience(ServerWorld world, @Nullable Entity attacker) {
    if ((!this.isExperienceDroppingDisabled())
        && (this.shouldAlwaysDropExperience()
            || (((this.playerHitTimer > 0) && this.shouldDropExperience())
  ...
  protected float getAttackKnockbackAgainst(Entity target, DamageSource damageSource) {
    ...
    return this.getWorld() instanceof ServerWorld serverWorld
        ? EnchantmentHelper.modifyKnockback(
            serverWorld, this.getWeaponStack(), target, damageSource, f)
        : f;
  }
  ...
  public ItemStack getBlockingItem() {
    if (!this.isUsingItem()) {
      ...
      BlocksAttacksComponent blocksAttacksComponent =
          this.activeItemStack.get(DataComponentTypes.BLOCKS_ATTACKS);
      if (blocksAttacksComponent != null) {
        int i =
            this.activeItemStack
                    .getItem()
                    .getMaxUseTime(this.activeItemStack, (LivingEntity) this.entityBridge)
  ...
  public float getDamageBlockedAmount(ServerWorld world, DamageSource source, float amount) {
    if (amount <= 0.0F) {
      ...
      if (itemStack == null) {
        ...
        BlocksAttacksComponent blocksAttacksComponent =
            itemStack.get(DataComponentTypes.BLOCKS_ATTACKS);
        if ((blocksAttacksComponent != null)
            && (!((Boolean)
                (blocksAttacksComponent.bypassedBy().map(source::isIn).orElse(false))))) {
  ...
  protected double getEffectiveGravity() {
    ...
    return bl && this.hasStatusEffect(StatusEffects.SLOW_FALLING)
        ? Math.min(this.getFinalGravity(), 0.01)
        : this.getFinalGravity();
  }

  public int getExperienceToDrop(ServerWorld world, @Nullable Entity attacker) {
    return EnchantmentHelper.getMobExperience(
        world, attacker, (LivingEntity) this.entityBridge, this.getExperienceToDrop(world));
  }
  ...
  public LivingEntity getPrimeAdversary() {
    if (this.attackingPlayer != null) {
      ...
      return this.attackerReference != null
          ? ((LivingEntity) (this.attackerReference.resolve(this.getWorld(), LivingEntity.class)))
          : null;
    }
  }
    ...
    if (this.isSpectator()) {
      ...
      if (blockState.isIn(BlockTags.CLIMBABLE)) {
        return true;
          ...
          && this.canEnterTrapdoor(blockPos, blockState)) {
        return true;
      } else {
        return false;
      }
  ...
  public boolean isInvulnerableTo(ServerWorld world, DamageSource source) {
    return this.isAlwaysInvulnerableTo(source)
        || EnchantmentHelper.isInvulnerableTo(world, (LivingEntity) this.entityBridge, source);
  }
    ...
    if (!(f <= 1.0E-5F)) {
      ...
      if (this.isSprinting()) {
        ...
        this.addVelocityInternal(
            new Vec3d((-MathHelper.sin(g)) * 0.2, 0.0, MathHelper.cos(g) * 0.2));
      }
    }
  }
    ...
    this.headYaw =
        ((float)
            (MathHelper.lerpAngleDegrees(
                1.0 / headTrackingIncrements, ((double) (this.headYaw)), serverHeadYaw)));
  }
  ...
  public void onDeath(DamageSource damageSource) {
    if ((!this.isRemoved()) && (!this.dead)) {
      ...
      if (this.getWorld() instanceof ServerWorld serverWorld) {
        if ((entity == null)
            || entity.onKilledOther(serverWorld, (LivingEntity) this.entityBridge)) {
  ...
  public void setAbsorptionAmount(float absorptionAmount) {
    this.setAbsorptionAmountUnclamped(
        MathHelper.clamp(absorptionAmount, 0.0F, this.getMaxAbsorption()));
  }
  ...
  protected void tickCramming() {
    List<Entity> list =
        this.getWorld().getCrammedEntities((LivingEntity) this.entityBridge, this.getBoundingBox());
    if (!list.isEmpty()) {
      if (this.getWorld() instanceof ServerWorld serverWorld) {
  ...
  public void tickMovement() {
    ...
    this.tickCramming();
    if (((this.getWorld() instanceof ServerWorld serverWorld) && this.hurtByWater())
  ...
  public void travel(Vec3d movementInput) {
    ...
    if (((this.isTouchingWater() || this.isInLava()) && this.shouldSwimInFluids())
  ...
  private void travelInFluid(Vec3d movementInput) {
    ...
    if (this.horizontalCollision
        && this.doesNotCollide(vec3d2.x, ((vec3d2.y + 0.6F) - this.getY()) + d, vec3d2.z)) {
  ...
  private void travelMidAir(Vec3d movementInput) {
    ...
    float f =
        (this.isOnGround())
            ? this.getWorld().getBlockState(blockPos).getBlock().getSlipperiness()
            : 1.0F;
    float g = f * 0.91F;
  ...
  public void move(MovementType type, Vec3d movement) {
    if (this.noClip) {
      this.setPosition(
          this.getX() + movement.x, this.getY() + movement.y, this.getZ() + movement.z);
    } else {
              ...
              this.getWorld()
                  .raycast(
                      new RaycastContext(
                          ...
                          ShapeType.FALLDAMAGE_RESETTING,
                          FluidHandling.WATER,
                          (LivingEntity) this.entityBridge));
  ...
  public boolean damage(ServerWorld world, DamageSource source, float amount) {
    if (this.isInvulnerableTo(world, source)) {
      ...
      if (source.isIn(DamageTypeTags.IS_FREEZING)
          && this.getType().isIn(EntityTypeTags.FREEZE_HURTS_EXTRA_TYPES)) {
        amount *= 5.0F;
      }
      if (source.isIn(DamageTypeTags.DAMAGES_HELMET)
          && (!this.getEquippedStack(EquipmentSlot.HEAD).isEmpty())) {
      ...
      if (bl2) {
        BlocksAttacksComponent blocksAttacksComponent =
            this.getActiveItem().get(DataComponentTypes.BLOCKS_ATTACKS);
        ...
        if (!source.isIn(DamageTypeTags.NO_KNOCKBACK)) {
          ...
          if (source.getSource() instanceof ProjectileEntity projectileEntity) {
            DoubleDoubleImmutablePair doubleDoubleImmutablePair =
                projectileEntity.getKnockback((LivingEntity) this.entityBridge, source);
      ...
      if (bl3) {
        ...
        for (StatusEffectInstance statusEffectInstance : this.getStatusEffects()) {
          statusEffectInstance.onEntityDamage(
              world, (LivingEntity) this.entityBridge, source, amount);
        }
      }
      ...
      if (source.getAttacker() instanceof ServerPlayerEntity serverPlayerEntityx) {
        Criteria.PLAYER_HURT_ENTITY.trigger(
            serverPlayerEntityx, (LivingEntity) this.entityBridge, source, amount, amount, bl);
      }
      return bl3;
    }
  }
  ...
  protected void fall(
    ...
    if (((this.getWorld() instanceof ServerWorld serverWorld) && onGround)
      ...
      if ((d > 0.0) && (!state.isAir())) {
        ...
        double g = this.getZ();
        BlockPos blockPos = this.getBlockPos();
        if ((landedPosition.getX() != blockPos.getX())
            || (landedPosition.getZ() != blockPos.getZ())) {
  ...
  private void playThornsSound(DamageSource damageSource) {
    if (damageSource.isOf(DamageTypes.THORNS)) {
      SoundCategory soundCategory =
          (this instanceof SlicedPlayerEntity) ? SoundCategory.PLAYERS : SoundCategory.HOSTILE;
      this.getWorld()
          .playSound(
              null,
              this.getPos().x,
  ...
  private boolean doesNotCollide(Box box) {
    return this.getWorld().isSpaceEmpty((LivingEntity) this.entityBridge, box)
