...
public abstract class SlicedEntity {
  ...
  @Nullable public Entity vehicle;
  private World world;
  public Vec3d pos;
  ...
  @Nullable public BlockState stateAtPos = null;
  ...
  protected Vec3d adjustMovementForCollisions(Vec3d movement) {
    ...
    List<VoxelShape> list =
        this.getWorld().getEntityCollisions((Entity) this.entityBridge, box.stretch(movement));
    Vec3d vec3d =
        (movement.lengthSquared() == 0.0)
            ? movement
            : Entity.adjustMovementForCollisions(
                (Entity) this.entityBridge, movement, box, this.getWorld(), list);
    ...
    if (((this.getStepHeight() > 0.0F) && (bl4 || this.isOnGround())) && (bl || bl3)) {
      ...
      List<VoxelShape> list2 =
          findCollisionsForMovement((Entity) this.entityBridge, this.world, list, box3);
      float f = ((float) (vec3d.y));
      float[] fs = collectStepHeights(box2, list2, this.getStepHeight(), f);
      for (float g : fs) {
        Vec3d vec3d2 =
            Entity.adjustMovementForCollisions(new Vec3d(movement.x, g, movement.z), box2, list2);
  ...
  protected static Vec3d adjustMovementForCollisions(
    ...
    if (collisions.isEmpty()) {
      ...
      for (Direction.Axis axis : getAxisCheckOrder(movement)) {
        ...
        if (d != 0.0) {
          double e =
              VoxelShapes.calculateMaxOffset(axis, entityBoundingBox.offset(vec3d), collisions, d);
  ...
  protected void applyMoveEffect(
      Entity.MoveEffect moveEffect, Vec3d movement, BlockPos landingPos, BlockState landingState) {
    ...
    if ((this.distanceTraveled > this.nextStepSoundDistance) && (!blockState.isAir())) {
      ...
      boolean bl3 =
          this.stepOnBlock(landingPos, landingState, moveEffect.playsSounds(), bl2, movement);
      if (!bl2) {
        bl3 |=
            this.stepOnBlock(blockPos, blockState, false, moveEffect.emitsGameEvents(), movement);
      }
    }
  }
  ...
  private boolean doesNotCollide(Box box) {
    return this.getWorld().isSpaceEmpty((Entity) this.entityBridge, box)
  ...
  private static List<VoxelShape> findCollisionsForMovement(
    ...
    Builder<VoxelShape> builder =
        ImmutableList.builderWithExpectedSize(regularCollisions.size() + 1);
  ...
  protected float getJumpVelocityMultiplier() {
    float f =
        this.getWorld().getBlockState(this.getBlockPos()).getBlock().getJumpVelocityMultiplier();
    float g =
        this.getWorld()
  ...
  protected boolean isAlwaysInvulnerableTo(DamageSource damageSource) {
    return ((this.isRemoved()
        ...
        || (damageSource.isIn(DamageTypeTags.IS_FALL)
            && this.getType().isIn(EntityTypeTags.FALL_DAMAGE_IMMUNE));
  }
  ...
  boolean isBeingRainedOn() {
    ...
    return this.getWorld().hasRain(blockPos)
        || this.getWorld()
            .hasRain(
                BlockPos.ofFloored(blockPos.getX(), this.getBoundingBox().maxY, blockPos.getZ()));
  }
  ...
  public void recalculateDimensions(EntityDimensions previous) {
    ...
    Optional<Vec3d> optional =
        this.world.findClosestCollision(
            (Entity) this.entityBridge,
            voxelShape,
            vec3d,
    ...
    if (optional.isPresent()) {
      this.setPosition(
          ((Vec3d) (optional.get())).add(0.0, (-entityDimensions.height()) / 2.0, 0.0));
    } else if ((entityDimensions.width() > previous.width())
        && (entityDimensions.height() > previous.height())) {
      ...
      if (optional2.isPresent()) {
        this.setPosition(
            ((Vec3d) (optional2.get())).add(0.0, ((-previous.height()) / 2.0) + 1.0E-6, 0.0));
      }
    }
  }
  ...
  public void setPos(double x, double y, double z) {
    if (((this.pos.x != x) || (this.pos.y != y)) || (this.pos.z != z)) {
      ...
      if (((i != this.blockPos.getX()) || (j != this.blockPos.getY()))
          || (k != this.blockPos.getZ())) {
  ...
  protected void tickBlockCollision() {
    if (this.currentlyCheckedCollisions.isEmpty()) {
      this.currentlyCheckedCollisions.add(
          new Entity.QueuedCollisionCheck(this.getLastRenderPos(), this.getPos()));
    }
  }
  ...
  protected void onSwimmingStart() {
    Entity entity =
        ((Entity)
            (Objects.requireNonNullElse(
                this.getControllingPassenger(), (Entity) this.entityBridge)));
    ...
    float g =
        Math.min(
            ...
            ((float)
                    (Math.sqrt(
                        (((vec3d.x * vec3d.x) * 0.2F) + (vec3d.y * vec3d.y))
                            + ((vec3d.z * vec3d.z) * 0.2F))))
                * f);
    if (g < 0.25F) {
      this.playSound(
          this.getSplashSound(),
